...
public class ZXingCodeEncodeUtils {
    ...
    public static void createZxingCodeUseLogoSaveToDisk(String content, int width, int height,
        ...
        try {
            BufferedImage bufferedImage = createZxingCodeUseLogo(content, width, height, logoStream);
            //保存图片到硬盘
            File file = new File(savePath);
            if (!file.exists()) {
                file.createNewFile();
        ...
        try {
            ...
            if (codeNormal != null) {
                ...
                if (logoStream != null) {
                    ...
                    //画白色边框
                    graphics2D.drawRoundRect(x, y, logo_width, logo_height, 15, 15);
                    //让画上的去的内容生效
                    graphics2D.dispose();
...
//        createZXingCodeSaveToDisk("cyh",400,400,"D:/cyh1.jpeg","JPEG");
//        InputStream logoStream = ZXingCodeEncodeUtils.class.getClassLoader().getResourceAsStream("./resources/images/avatar1.jpg");
        //资源跟src同目录
//        InputStream logoStream = Thread.currentThread().getContextClassLoader().getResourceAsStream("avatar.jpg");

        // getClassLoader()表示路径前不用加/   去掉getClassLoader()表示路径前要加/
        InputStream logoStream = ZXingCodeEncodeUtils.class.getClassLoader().getResourceAsStream("src/avatar.jpg");

        createZxingCodeUseLogoSaveToDisk("cyh1234511111", 400, 400, "D:/cyhlogo.jpeg", "JPEG", logoStream);
    }
}
